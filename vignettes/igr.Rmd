...
There are many R packages facilitating [spatial data analysis and visualisation](https://cran.r-project.org/view=Spatial). The igr package is designed to simplify using datasets containing Irish grid references in R. It translates Irish grid references to and from Irish Grid coordinates (X and Y), or to and from [sf](https://r-spatial.github.io/sf/) (simple feature) objects that other R packages can work with. The igr package supports precisions of Irish grid reference from 100 km to 1 m including tetrads, mixes of precision, and Irish grid references with or without whitespace between the letters, eastings and northings.
...
Irish grid references can be converted to coordinates in any coordinate reference system, e.g. Longitude and Latitude (WGS 84).
```{r example-igr-poi-wgs84}
...
```{r example-igr-poi-plot, message=FALSE, fig.alt="A map of Ireland with a dot at the south west corner of each sample grid reference."}
...
```{r  example-igr-pol-plot, fig.alt="A map of Ireland with polygons spanning each sample grid reference. The polygons range in size from 100 km square to 1 m square."}
if (exists("ie_uk_sf")) {
